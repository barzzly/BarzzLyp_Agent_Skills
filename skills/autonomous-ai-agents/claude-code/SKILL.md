...
Pass `--model` to route tasks to suitable high-capability coding models (e.g. `--model sonnet`, `--model opus`, or OpenAI/custom models supported by local environment configuration).

**Pterodactyl & Server Deployments:**
When working with Pterodactyl panels, prefer client API endpoints (`/api/client/servers/{id}/...`) for server power/backups and direct SFTP key authentication (`sftp -P <port> <user>@<host>`) for high-volume file management to avoid web bot detection / Turnstile WAF barriers.
...
13. **Verify custom-model routing from output, not CLI intent.** Inspect the stream `system/init.model` and final `modelUsage` before claiming a requested model ran. Global `ANTHROPIC_DEFAULT_OPUS_MODEL` can redirect a custom `--model`; override routing only for that invocation with `--settings` JSON setting `model`, `env.ANTHROPIC_MODEL`, and relevant default model variables to the requested ID. Probe once and check actual output; never silently substitute models or rewrite global settings.
16. **Repeated HTTP 504 with no source writes can mean an oversized generation is timing out.** Preserve completed files, stop the stuck attempt after confirming repeated retries, then run a fresh same-model invocation scoped to the missing module. Request small Write/Edit chunks (about 5 KB) with a unique append marker and syntax checks between chunks; do not silently switch models. Existing subagent artifacts can be reused after independent verification.
14. **Background launch may fail before command execution when session bus is unavailable.** If terminal background reports `Failed to connect to bus` and expected log files never appear, use an isolated named tmux session running the print-mode process, with project-local logs. Read logs for progress and completion; remove only that tmux session afterward.
15. **Video-driven WebGL work requires visual QA beyond successful builds.** Extract timed reference frames with ffmpeg, inspect them, pass concrete geometry/camera/material details to the coding agent. Verify rendered screenshots, model attachments, working controls, local-only assets and mobile input. A CSS `display:grid/flex` rule can override HTML `hidden`; test computed visibility and include a `[hidden] { display:none !important; }` safeguard when appropriate.
