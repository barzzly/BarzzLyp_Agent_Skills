...
- **Multi-Version Resource Pack & ModelEngine Overlay Compatibility (1.20.1 to 26.x+)**:
  ...
  - In `pack.mcmeta`, set `"pack_format": 15` and `"supported_formats": {"min_inclusive": 15, "max_inclusive": 32767}` so 1.20.1 loads natively without client-side "incompatible version" warnings while 1.20.2+ clients recognize compatibility through the highest modern versions.
  - When extending overlays for newer snapshot/release targets (e.g. `26.1` format 84, `26.2` format 88, `26.3` format 92+), provide corresponding `modelengine_26_x` overlay entries with updated core shaders (`entity.vsh` with `#if defined(PER_FACE_LIGHTING) || !defined(NO_CARDINAL_LIGHTING)` light checks).
