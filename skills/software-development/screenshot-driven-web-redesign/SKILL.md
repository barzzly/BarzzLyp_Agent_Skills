...
- Animate fast-changing multiplayer readouts between confirmed server snapshots with `requestAnimationFrame`; do not predict hidden end times. Keep settlement server-authoritative, freeze on stale data, and test sampled displayed increments plus terminal-state snapping.
- For partial demo settlements, track remaining principal and cumulative payout separately; test odd-amount rounding, duplicate partial requests, full remainder payout, and preserving already credited payouts after a round ends. Participant lists must distinguish partial, full, and partial-then-lost states using explicit labels plus credited/remaining amounts.
- Verify tier-color computed styles separately in live readouts, compact history strips, and portaled dialogs; a generic chip rule can lose to a more-specific legacy selector even when the same class works elsewhere. Test both sides of every numeric tier boundary.
- Keep fake `Date.now()` values integer in Vite QA harnesses; fractional milliseconds corrupt HMR timestamp URLs and can trigger an `Invalid loader value` error.
- For Noesan Man art, source existing creator-made cartoon assets with verified reuse licenses instead of drawing/generating replacement characters; the user rejects agent-made diver art. Preserve attribution and modification notices. Current diver source: OpenGameArt Scuba Diver by Fellowship Of the Game, CC BY 4.0. Keep terrain behind the full silhouette and retain blue/gold scene accents.
- For downloaded sprite sheets, derive occupied alpha bounds before cropping rather than dividing the full canvas evenly; keep fixed frame anchors and verify transparent margins on every frame. CSS reduced-motion rules do not pause animated WebP/GIF playback: swap to a static image and verify the animated image is hidden under emulated `prefers-reduced-motion: reduce`.
- When renaming a public page, update desktop/mobile navigation, headings, accessible labels, and visible rules together; redirect legacy routes and test both old and new URLs because changing only the heading leaves stale entry points.
- When changing numeric gameplay limits, update server validation, client eligibility, input bounds, and presets together; test both inclusive limits, adjacent invalid values, insufficient balance, and settlement at the maximum because UI-only bounds do not constrain direct API requests.
- Test long history browsers against actual isolated engine rollovers with a fake clock, never seed invented production history; verify every numbered page and newest-first IDs, then test production against only its real available rounds.
- Bring browser tabs to foreground before checking animation completion or dialog unmount/focus restoration; background tabs can pause animation-end events while DOM still has `data-state="closed"`.
- Keep SVG placement transforms on an outer `<g>` and animation transforms on a nested `<g>`; CSS transforms on the same node override SVG translation and move characters off their intended anchor.
- For above-water waiting poses, use a static licensed sprite, anchor its fins to a visible float, and animate entry only on the diving phase. Check mobile tagline clearance using actual sprite bounds; a rotated full-size swim sprite can overlap countdown copy. Hide surface overlays immediately for ended/reduced-motion states.
- Set `min-width: 0` on CSS grid children containing history strips or long text; `overflow: hidden` on the arena can hide an oversized internal grid even when document scroll width appears correct. Verify each control bounding box against viewport edges.
- Change desktop navigation, hamburger visibility, and header grid breakpoints together when adding links. Verify tablet widths explicitly; mixing `lg:flex` navigation with `md:hidden` hamburger removes both navigation paths.
- Preview new game/demo routers in an isolated Express + Vite harness rather than starting a second full production app; full bootstrap may activate RCON, payment retries, and other background jobs. Keep demo credits isolated from store or game-server economies.
